...
object ServiceHelper {
    ...
    /**
     * 각 Service에 연결하기 위해 Parameter 생성과 공통의 Response를 처리 한다.
     */
...
//                    }
        ...
        return { req: HttpServerRequest, resp: HttpServerResponse ->
            /** return resp.sendString(req.receive().aggregate()) */
...
//                }
                .map {
                    ...
                    if(it["status"] == 200 && path.lastIndexOf("csv") > -1 ){
                        ...
                        // Todo data[0].keys 를 하지 않는 이유 / row 값들중 중간에 없는키가 있을 수 있음
                        ...
                        // TODO 파라미터로 받아야 하는지 토큰에서 가져와야 하는지 정하지 못함.
